...
opts = OptionParser.new do |opts|
  ...
  opts.on("-i", "--iphone", "Append iphone to folder name.") do |i|
    options.directory_suffix = " (iphone)"
  end
end
...
source = ARGV[0] || "/Volumes/Card_Reader/"
destination = ARGV[1] || "/Users/bjhess/pictures/"
...
Dir["#{source}/**/*.{jpg,JPG,jpeg,JPEG,raw,RAW,tif,TIF,gif,GIF,png,PNG}"].each do |file_name|
  ...
  new_dir_name = exif_info.date_time_original.strftime("%Y-%m-%d") + options.directory_suffix
  destination_dir = destination + new_dir_name
  FileUtils.mkdir destination_dir unless FileTest.exist?(destination_dir)
  new_file_name = exif_info.date_time_original.strftime("%Y-%m-%d(%H.%M.%S)")
...
puts "DON'T FORGET ABOUT VIDEOS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
